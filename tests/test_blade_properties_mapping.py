...
from apps.PumpForge3D.app.controllers.blade_properties_binder import (
    map_blade_inputs_to_inducer_kwargs,
)
...
def test_mapping_updates_inducer_fields():
    ...
    assert updates["incidence_in"] == math.radians(3.0)
    assert updates["slip_out"] == math.radians(6.0)
    assert updates["thickness_in"] == (2.0 + 3.0) / 2.0 / 1000.0
    assert updates["thickness_out"] == (1.5 + 2.5) / 2.0 / 1000.0
